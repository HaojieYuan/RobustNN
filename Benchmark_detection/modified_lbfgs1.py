...
class LBFGS(Attack):
  ...
  def generate(self, x, **kwargs):
    ...
    attack = LBFGS_impl(
        ...
        self.targeted_attack, self.binary_search_steps, 
        self.max_iterations, self.initial_const,
        self.clip_min, self.clip_max, nb_classes, self.batch_size)

    result = attack.attack(x, self.y_target)
    
    # py_func does not support None in the input list
    def lbfgs_wrap(adv_x):
      """
      Wrapper creating Tensorflow interface for use with py_func
      """
      return np.array(adv_x, dtype=self.np_dtype)
    
    wrap = tf.py_func(lbfgs_wrap, [result], self.tf_dtype)
  ...
  def parse_params(self,
    ...
    self.targeted_attack = self.y_target is not None
...
class LBFGS_impl(object):
  ...
  :param targeted_attack: Indicate the attack is targeted or untargeted.
  :param binary_search_steps: The number of times we perform binary
                              search to find the optimal tradeoff-
                              constant between norm of the purturbation
                              and cross-entropy loss of classification.
  ...
  def attack(self, x_val, targets=None):
    """
